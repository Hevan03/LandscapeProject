...
const ServicePayments = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            // Map UI fields to backend contract in paymentModel.js
            const paymentData = {
                // Use test ObjectIds when URL params indicate test or are missing
                orderId: serviceDetails.serviceId && serviceDetails.serviceId.startsWith('test')
                    ? '507f1f77bcf86cd799439011'
                    : serviceDetails.serviceId,
                customerId: customerId
                    ? (customerId.startsWith('test') ? '507f1f77bcf86cd799439012' : customerId)
                    : '507f1f77bcf86cd799439012',
                amount: amount,
                method: 'BankSlip',
                bankSlipUrl: bankSlip ? bankSlip.name : undefined,
                // Preserve stage info inside notes for audit
            ...
            <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-2xl">
                ...
                <div className="bg-gray-100 p-4 rounded-lg mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <p className="text-sm font-semibold">Service ID: <span className="text-green-700">{serviceDetails.serviceId}</span></p>
                            <p className="text-sm">Customer: <span className="font-medium">{serviceDetails.customerName}</span></p>
                        </div>
                        <div>
                            <p className="text-sm">Total Project Cost: <span className="font-medium text-lg">LKR {serviceDetails.totalProjectCost.toLocaleString()}</span></p>
                            <p className="text-sm">Remaining Balance: <span className="font-medium text-lg text-red-600">LKR {balanceAmount.toLocaleString()}</span></p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="space-y-2">
                            {paymentHistory.map((payment) => (
                                <div key={payment.id} className="flex justify-between items-center bg-white p-2 rounded">
                                    ...
                                    <span className={`badge badge-sm ${payment.status === 'Pending' ? 'badge-warning' : 'badge-success'}`}>
                                        {payment.status}
                                    </span>
                                </div>
                            ))}
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="form-control">
                            ...
                            <select
                                ...
                                value={paymentStage}
                                onChange={handlePaymentStageChange}
                                required
                            >
                                <option disabled value="">Choose Stage</option>
                        ...
                        <div className="form-control">
                            ...
                            <input
                                ...
                                value={`LKR ${amount.toLocaleString()}`}
                                readOnly
                            />
                        </div>
                    </div>
                    ...
                    <div className="form-control mt-6">
                        ...
                        <div className="flex items-center space-x-4">
                            ...
                            />
                            {bankSlip && (
                                <span className="text-sm text-gray-500">{bankSlip.name}</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex justify-between items-center mt-8">
                        <button type="submit" className="btn bg-green-700 hover:bg-green-800 text-white font-bold py-3 px-8 rounded-full shadow-lg transition-transform transform hover:scale-105">
                            Confirm Payment
                        </button>
                        
                        <div className={`badge badge-lg p-4 ${status === 'Pending' ? 'badge-warning' : status === 'Completed' ? 'badge-success' : 'badge-error'} text-white`}>
