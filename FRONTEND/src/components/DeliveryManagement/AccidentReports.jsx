...
import { getAllAccidentReports, updateAccidentReportStatus } from "../../api/accidentReportApi";
    ...
    <div className="bg-gray-100 min-h-screen p-8 font-sans">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          />
          <select name="status" className="select select-bordered rounded-lg" onChange={handleFilterChange}>
            <option value="">Filter by Status</option>
            <option value="Reported">Reported</option>
            <option value="Under Investigation">Under Investigation</option>
            <option value="Resolved">Resolved</option>
          </select>
          <select name="driver" className="select select-bordered rounded-lg" onChange={handleFilterChange}>
          ...
          <select name="vehicle" className="select select-bordered rounded-lg" onChange={handleFilterChange}>
              ...
              {filteredReports.length > 0 ? (
                filteredReports.map((report) => (
                  <tr key={report._id} className="hover">
                    ...
                    <td>
                      <div className={`badge ${getStatusBadge(report.status)} text-white`}>{report.status}</div>
                    </td>
      ...
      {isDetailsModalOpen && selectedReport && (
        <dialog open className="modal">
          <div className="modal-box bg-white p-6 rounded-xl shadow-xl">
            <h3 className="font-bold text-lg mb-4">Accident Report Details ({selectedReport._id})</h3>
            <div className="flex flex-col gap-4">
              ...
              <p>
                <strong>Delivery ID:</strong> {selectedReport.deliveryId || "N/A"}
              </p>
              <p>
                <strong>Date & Time:</strong> {selectedReport.time ? new Date(selectedReport.time).toLocaleString() : "N/A"}
              </p>
              <div className={`badge badge-lg text-white ${getStatusBadge(selectedReport.status)}`}>{selectedReport.status}</div>
              <p className="mt-4">
              ...
              {selectedReport.photo && (
                <>
                  ...
                  <img src={selectedReport.photo} alt="Accident" className="w-full rounded-lg shadow-md mt-2" />
                </>
              )}
              ...
              <div className="flex justify-end gap-2 mt-4">
                {selectedReport.status !== "Under Investigation" && (
                  <button className="btn btn-sm btn-warning" onClick={() => handleUpdateStatus(selectedReport._id, "Under Investigation")}>
                    <FaHourglassHalf className="mr-2" /> Under Investigation
                  </button>
                )}
                {selectedReport.status !== "Resolved" && (
                  <button className="btn btn-sm btn-success" onClick={() => handleUpdateStatus(selectedReport._id, "Resolved")}>
                    <FaCheck className="mr-2" /> Mark as Resolved
                  </button>
                )}
                <button className="btn btn-sm btn-info" onClick={() => handleNotifyCustomer(selectedReport.deliveryId)}>
