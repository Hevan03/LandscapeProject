...
const { landscapeId } = useParams();
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      if (!landscapeId) {
        setError("No landscape ID provided.");
        setIsLoading(false);
        return;
      }
      try {
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center h-screen"><p>Loading your project progress...</p></div>;
  }
  ...
  return (
    <div className="bg-gray-100 min-h-screen pt-24 pb-12 px-4">
      <div className="max-w-2xl mx-auto">
        <h1 className="text-4xl font-bold text-center text-gray-900 mb-8">My Project Progress</h1>
              ...
              <div key={post._id} className="bg-white rounded-lg shadow-xl overflow-hidden">
                ...
                <div className="p-6 border-b border-gray-200">
                  ...
                  <div className="mt-4 space-y-2 text-gray-600">
                    <div className="flex items-center"><TreePine size={16} className="mr-3 text-green-600" /> <span>Project: <strong>{post.landscapeId?.name || 'N/A'}</strong></span></div>
                    <div className="flex items-center"><HardHat size={16} className="mr-3 text-green-600" /> <span>Handled by: <strong>{post.landscaperId?.name || 'N/A'}</strong></span></div>
                  </div>
                </div>
