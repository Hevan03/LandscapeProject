  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setError(
          "Failed to fetch the progress post. It may have been deleted or the link is incorrect."
        );
        console.error(err);
      } finally {
        setIsLoading(false);
  ...
  const handleDelete = async () => {
    ...
    if (isConfirmed) {
      try {
        ...
        setTimeout(() => {
          navigate(`/addprogress/${post.landscapeId._id}`);
        }, 2000);
      } catch (err) {
        ...
        <div className="p-6 border-b border-gray-200">
          ...
          <div className="mt-4 space-y-2 text-gray-600">
            ...
            <div className="flex items-center">
              ...
              <span>
                Customer: <strong>{post.customerId.name}</strong>
              </span>
            </div>
            <div className="flex items-center">
              <HardHat size={16} className="mr-3 text-green-600" />{" "}
              <span>
                Landscaper: <strong>{post.landscaperId.name}</strong>
              </span>
            </div>
          </div>
        </div>
