    ...
    <div className="bg-gray-100 min-h-screen pt-24 pb-12 px-4">
      <form onSubmit={handleFormSubmit} className="max-w-3xl mx-auto bg-white rounded-lg shadow-xl overflow-hidden">
        ...
        <div className="p-6 border-b">
          ...
          {projectDetails ? (
            <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4 text-sm bg-green-50 p-4 rounded-md border border-green-200">
               <div className="flex items-center"><TreePine size={16} className="mr-2 text-green-700" /><span>{projectDetails.landscapeName}</span></div>
               <div className="flex items-center"><User size={16} className="mr-2 text-green-700" /><span>{projectDetails.customerName}</span></div>
               <div className="flex items-center"><HardHat size={16} className="mr-2 text-green-700" /><span>{projectDetails.landscaperName}</span></div>
            </div>
          ) : (
        ...
        <div className="p-6 border-t">
            ...
            <input id="postName" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., Week 1 - Foundation Work" className="w-full p-3 border rounded-md focus:ring-2 focus:ring-green-500" required />
        </div>
        ...
        <div className="p-6 border-t">
          ...
          <textarea id="description" value={description} onChange={(e) => setDescription(e.target.value)} className="w-full h-32 p-3 border rounded-md focus:ring-2 focus:ring-green-500" placeholder="Provide a summary of the progress..."></textarea>
        </div>
          ...
          <div className="space-y-3 mb-4">
              {tasks.map((task, index) => (
                  <div key={index} className="flex items-center justify-between p-2 bg-gray-50 rounded-md">
                      ...
                      <button type="button" onClick={() => handleDeleteTask(index)}><Trash2 size={20} className="text-red-500 hover:text-red-700"/></button>
                  </div>
              ))}
          </div>
          <div className="flex items-center">
            <input type="text" value={newTaskName} onChange={(e) => setNewTaskName(e.target.value)} placeholder="Enter new task name" className="flex-grow p-2 border rounded-l-md focus:ring-2 focus:ring-green-500"/>
            <button type="button" onClick={handleAddTask} className="bg-gray-800 text-white p-2 rounded-r-md hover:bg-black"><Plus size={24}/></button>
          </div>
        </div>

        <div className="p-6 border-t">
          <RequiredLabel label="Upload Images (Exactly 5)" />
          <input type="file" onChange={handleImageChange} className="w-full p-2 border rounded-md file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100" multiple accept="image/*" required />
        ...
        <div className="p-6 bg-gray-50 border-t">
          <button type="submit" disabled={isSubmitting} className="w-full py-3 bg-green-600 text-white font-bold rounded-lg shadow-md hover:bg-green-700 disabled:bg-gray-400">
