...
import landscaperModel from "../../Models/landscape/landscaperModel.js";
...
export async function createLandscaper(req, res) {
  try {
    const { name, contact, phone, email, address, Employee_Image, specialties, availability, password, status } = req.body;

    // Basic validation
    if (!name || !contact || !phone || !email || !password) {
      return res.status(400).json({
        message: "name, contact, phone, email, and password are required",
      });
    }
    ...
    res.status(201).json({
      message: "Landscaper created successfully",
      data: newLandscaper,
    });
  } catch (error) {
    ...
    if (error.code === 11000 && error.keyPattern?.serviceNum) {
      return res.status(400).json({
        message: "Service number already exists. Please use a different service number.",
      });
    }
...
export async function updateLandscaperProfile(req, res) {
  try {
    const { id } = req.params;
    const landscaper = await landscaperModel.findById(id);
    ...
    // Update basic fields
    landscaper.name = req.body.name || landscaper.name;
    landscaper.phone = req.body.phone || landscaper.phone;
    landscaper.email = req.body.email || landscaper.email;
    landscaper.address = req.body.address || landscaper.address;

    console.log("Passed specialties:", req.body.specialties);
    // Handle specialties (convert from JSON string)
    if (req.body.specialties) {
      landscaper.specialties = JSON.parse(req.body.specialties);
    }

    // Handle availability (convert from JSON string)
    if (req.body.availability) {
      landscaper.availability = JSON.parse(req.body.availability);
    }

    // Handle profile image
    if (req.file) {
      landscaper.Employee_Image = req.file.path;
    }

    await landscaper.save();
    ...
    res.status(500).json({
      message: "Error updating landscaper profile",
      error: error.message,
    });
  }
}
