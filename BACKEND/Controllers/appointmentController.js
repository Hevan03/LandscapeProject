...
export async function createAppointment(req, res) {
  try {
    const {
      customer, landscaper, appointmentDate, timeSlot, contactNumber,
      customerAddress, siteAddress, siteLocation, siteArea
    } = req.body;
...
export async function getAppointmentsForCustomer(req, res) {
    ...
    try {
        const appointments = await AppointmentModel.find({ customer: customerId })
            .populate('landscaper', 'name contact')
...
export async function getAppointmentsForLandscaper(req, res) {
    ...
    try {
        const appointments = await AppointmentModel.find({ landscaper: landscaperId })
            .populate('customer', 'name')
            .sort({ appointmentDate: -1 });
        res.status(200).json(appointments);
    } catch (error) {
