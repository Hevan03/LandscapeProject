...
import landscapeModel from "../Models/landscapeModel.js";
...
export async function getAllProgress(req, res) {
  try {
    const progresses = await progressModel.find().sort({ createdAt: -1 })
...
export async function createProgress(req, res) {
  try {
    const { landscapeId, name, description, tasks } = req.body;
     ...
     const project = await landscapeModel.findById(landscapeId).select("customerId landscaperId");
    if (!landscapeId) {
...
export async function getProgressForLandscape(req, res) {
  ...
  try {
    const posts = await progressModel.find({ landscapeId: landscapeId })
      .sort({ createdAt: -1 })
      .populate("landscaperId", "name")
      // --- THIS IS THE FIX ---
