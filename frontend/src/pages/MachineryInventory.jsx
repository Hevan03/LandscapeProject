...
const AddMachineryForm = ({ onAdd, onClose }) => {
  ...
  const handleChange = (e) => {
    ...
    if (
      ["rentalPricePerDay", "defaultDurationDays", "penaltyPerDay", "quantity"].includes(
        name
      )
    ) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5001/api/machinery/machines",
        data,
...
function MachineryInventory() {
  ...
  const handleUpdateSuccess = (updatedMachine) =>
    setMachines((prev) =>
      prev.map((m) => (m._id === updatedMachine._id ? updatedMachine : m))
    );
    ...
    <div>
      ...
      {machines.length === 0 ? (
        <p className="text-center text-gray-500">
          No machinery found in the inventory.
        </p>
      ) : (
      ...
      {/* Modals */}
      {isFormOpen && (
        <AddMachineryForm onAdd={handleAddSuccess} onClose={handleCloseForm} />
      )}
