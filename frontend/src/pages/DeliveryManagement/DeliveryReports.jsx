...
const DeliveryReports = () => {
    ...
    const handleExportTablePdf = async () => {
        try {
            ...
            const body = reports.map(r => [
                ...
                r.status || 'N/A',
                r.orderId?.deliveryAssignedDate ? format(parseISO(r.orderId.deliveryAssignedDate), 'yyyy-MM-dd') : (r.assignedDate ? format(parseISO(r.assignedDate), 'yyyy-MM-dd') : ''),
                r.orderId?.deliveredAt ? format(parseISO(r.orderId.deliveredAt), 'yyyy-MM-dd') : '',
            ...
            const filename = `delivery-table-${new Date().toISOString().slice(0,10)}.pdf`;
            doc.save(filename);
    ...
    const processReportsForCharts = (data) => {
        ...
        setDeliveriesData(Object.keys(monthlyCounts).map(month => ({ name: month, count: monthlyCounts[month] })));
        ...
        setDriverData(Object.keys(driverCounts).map(name => ({ name, deliveries: driverCounts[name] })));
    ...
    return (
        <div className="min-h-screen" id="delivery-report-container">
                    ...
                    <div className="form-control">
                        ...
                        >
                            ...
                            {vehicles.map(vehicle => (
                                <option key={vehicle._id} value={vehicle._id}>{vehicle.vehicleNo}</option>
                            ))}
                        </select>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                {/* Main Table */}
                <div id="delivery-report-summary" className="bg-white p-6 rounded-2xl shadow-lg">
                    <div className="flex justify-between items-center mb-4">
                        <h2 className="text-xl font-semibold text-gray-700">Delivery Summary Table</h2>
                        <div className="flex gap-2">
                            <button onClick={handleExport} className="btn bg-green-700 hover:bg-green-800 text-white rounded-lg shadow-lg">
                                <BiExport className="mr-2" />
                                Export PDF (Card)
                            </button>
                            <button onClick={handleExportTablePdf} className="btn btn-primary text-white">
                                Export Table PDF
                            </button>
                        ...
                        <table className="table w-full">
                            ...
                            <tbody>
                                ...
                                <tr className="hover">
                                    ...
                                    <td>{reports.filter(r => r.status === 'Delivered').length}</td>
                                </tr>
                                <tr className="hover">
                                    <td>Failed/Cancelled Deliveries</td>
                                    <td>{reports.filter(r => r.status === 'Canceled' || r.status === 'Failed').length}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
